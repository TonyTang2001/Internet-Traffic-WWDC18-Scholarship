...
/*:
 ...
 - - -
 ## Now Build Your Own HTTP Connection !
 Notice: Code Operations for User Device and Web Server respectively(in their own functions).
 
 1. User sends out a [HTTP-Request](glossary://HTTP-Request) by **userSendRequest()**
 
 2. Server receives request by **serverReceivingRequest()**
 
 3. Server sends [Files](glossary://Files) by **serverSendFiles()**
 
 4. User Device receive [Files](glossary://Files) by **userReceiveFiles()**
 
 5. User Device [Decode](glossary://Decode) and Display WebPage by **userDecodeAndDisplay()**
...
class MyViewController : UIViewController {
    ...
    override func loadView() {
        ...
        func forMissingProcess(atSide: Int) {
            ...
            if atSide == 1 {
                errorLabel.text = " Server Side Error! "
            }
        ...
        func serverSendFiles() {
            serverSendFilesv = true
        }
        ...
        func serverToUser() {
            if serverSendFilesv == true && runningOk == true {
